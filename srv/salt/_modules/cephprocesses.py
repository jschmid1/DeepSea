...
class MetaCheck(object):
    ...
    def _missing_osds(self):
        ...
        """
        up_osds = [str(x.osd_id) for x in self.filter_for('ceph-osd')]
        return list(set(self.expected_osds) - set(up_osds))

    def _insufficient_osd_count(self):
        """
        Check if the sufficient number of OSDs are up
        """
        up_osds = [x.osd_id for x in self.filter_for('ceph-osd')]
        if len(self.expected_osds) > len(up_osds):
            if not self.quiet:
                # pylint: disable=line-too-long
                log.error("{} OSDs not running: {}".format(len(self._missing_osds), self._missing_osds))
                log.error("Found less OSDs then expected. Expected {} | Found {}".format(len(self.expected_osds), len(up_osds)))
    ...
    def report(self):
        """
        Format the strctures so that salt understands it
