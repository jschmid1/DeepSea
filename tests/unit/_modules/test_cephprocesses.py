...
class TestCephprocessesProcInfo():
    ...
    def test_map_open_proc_to_osd_id_multi_digits(self):
        """
    ...
    def test_map_open_proc_to_osd_id_multi_raises(self):
        """
...
class TestMetaCheck():
    ...
    def test_filter_ceph_osd_multi(self, mc):
        expected_len = 10
...
class TestInstanceMeth():
    ...
    @mock.patch('srv.salt._modules.cephprocesses.MetaCheck.add')
    @mock.patch('srv.salt._modules.cephprocesses.ProcInfo')
    def test_check_3(self, proc_mock, meta_mock, mock_check_osds, meta_check_invert, meta_check_absent, mock_report, psutil_mock):
        """
        Parameterized test to verify that roles get passed down to methods
        """
        role = 'storage'
        ...
        proc_mock.assert_called_with('proc1')
        # ANY instance of ProcInfo
        meta_mock.assert_called_with(ANY, role)
        meta_check_invert.assert_called_with(role)
        meta_check_absent.assert_called_with(role)
        mock_check_osds.assert_called_once
        mock_report.assert_called
